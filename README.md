...
#### 2.1 詳細な処理フロー（recorded_atベースの判定）

**重要な仕様**: 処理対象の判定は`recorded_at`フィールドを基準に行います。

```
[1. 日付による絞り込み]
- 入力: device_id="xxx", date="2025-07-19"
- SQLクエリ条件:
  - device_id = 'xxx'
  - recorded_at >= '2025-07-19T00:00:00'
  - recorded_at < '2025-07-20T00:00:00'
- 結果: 2025-07-19の00:00:00から23:59:59.999...までの全レコード
...
[3. 順次処理]
- pendingレコードを1件ずつ処理:
  1. S3からfile_pathを使って音声ファイルをダウンロード
  2. Whisperで文字起こし
  3. vibe_whisperテーブルに結果を保存
  4. audio_filesのtranscriptions_statusを'completed'に更新

[4. レスポンス]
- 処理結果サマリーを返却:
  - total_files: 該当日の全ファイル数
  - already_completed: 既に処理済みのファイル数
  - pending_processed: 今回処理したファイル数
  - errors: エラー発生数
```
...
// 処理フロー
1. audio_filesテーブルから2025-07-19のレコードを検索
   → 2件発見（00:30と01:30のデータ）
2. その中でtranscriptions_status='pending'のものを抽出
   → 2件とも'pending'
3. 各ファイルを順次処理:
   - files/xxx/2025-07-19/00-30/audio.wav → 処理完了
   - files/xxx/2025-07-19/01-30/audio.wav → 処理完了
4. 処理結果を返却
```
